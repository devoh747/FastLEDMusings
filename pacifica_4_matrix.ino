...
uint8_t XY( uint8_t x, uint8_t y)
{
  ...
  if( y & 0x01) {
    ...
    uint8_t reverseX = (NUM_ROWS - 1) - x;
    i = (y * NUM_ROWS) + reverseX;
  } else {
...
void setup() {
  ...
  FastLED.addLeds<LED_TYPE,DATA_PIN,COLOR_ORDER>(gLEDs, NUM_LEDS)
  ...
  EVERY_N_MILLISECONDS( 20 ) {
    ...
    for ( uint8_t down = 0; down < NUM_ROWS; down++) {
      for (uint8_t across = 0; across < NUM_COLS; across++) {
        gLEDs[XY(down,across)] = gOneRowOfLEDs[across];
      }
    }   
    FastLED.show();
  }
